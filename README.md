...
> TODO:
>
> - Update guidance on report-id. Consider removing it's recommendation and the
>   paragraph that demonstrates the inline status updates (which is what
...
jobs:
  bench_1:
    ...
    steps:
      ...
      - name: Run tachometer and generate results file
      - run: npm run tach --config benchmarks.json --json-file results-1.json
